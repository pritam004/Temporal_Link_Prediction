...
parser.add_argument('--node_dim', type=int, default=25, help='Dimentions of the node embedding')
parser.add_argument('--time_dim', type=int, default=25, help='Dimentions of the time embedding')
...
def eval_one_epoch(hint, tgan, sampler, src, dst, ts, label):
    ...
    with torch.no_grad():
        ...
        for k in range(num_test_batch):
            ...
            _,_,pos_prob, neg_prob = tgan.contrast(src_l_cut, dst_l_cut, dst_l_fake, ts_l_cut, NUM_NEIGHBORS)
...
g_df = pd.read_csv('./mastercard_trans_data_train.csv',index_col=None)
# e_feat = np.load('./processed/ml_{}.npy'.format(DATA))
# n_feat = np.load('./processed/ml_{}_node.npy'.format(DATA))

e_feat=np.random.rand(g_df.shape[0],25)
n_feat=pd.read_csv('/content/drive/MyDrive/NXM/EvolveGCN/data/combined_feat.csv',index_col=None).values
n_feat=np.random.rand(g_df.source.max()+1,25)

val_time, test_time = list(np.quantile(g_df.discrete_time, [1, 1]))


src_l = g_df.cc_id.values
dst_l = g_df.merchant_id.values
e_idx_l = np.array([i for i in range(g_df.shape[0])])
label_l = np.array([1 for i in range(g_df.shape[0])])
ts_l = g_df.discrete_time.values
...
total_node_set = set(np.unique(np.hstack([g_df.cc_id.values, g_df.merchant_id.values])))
...
# mask_src_flag = g_df.cc_id.map(lambda x: x in mask_node_set).values
# mask_dst_flag = g_df.merchant_id.map(lambda x: x in mask_node_set).values
...
train_dst_l = dst_l
train_ts_l = ts_l
train_e_idx_l = e_idx_l
train_label_l = label_l

val_time, test_time = list(np.quantile(g_df.discrete_time, [0.50, 0.85]))
...
for epoch in range(NUM_EPOCH):
    ...
    for k in tqdm(range(num_batch)):
        ...
        optimizer.zero_grad()
        tgan = tgan.train()
        s_emb,t_emb,pos_prob, neg_prob = tgan.contrast(src_l_cut, dst_l_cut, dst_l_fake, ts_l_cut, NUM_NEIGHBORS)
        s_emb_list.append(s_emb.detach().cpu().numpy())
        t_emb_list.append(t_emb.detach().cpu().numpy())
    ...
    np.save(f"./s_emb{epoch}",np.array(s_emb_list))
    np.save(f"./t_emb{epoch}",np.array(t_emb_list))
